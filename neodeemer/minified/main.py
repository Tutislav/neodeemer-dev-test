...
from kivymd.uix.list import(IconLeftWidget,IconRightWidget,ILeftBody,OneLineAvatarIconListItem,TwoLineAvatarIconListItem,TwoLineIconListItem)
...
class Neodeemer(MDApp):
 ...
 selected_tracks=[]
 download_queue=[]
 download_queue_info={"position":0,"downloaded_b":0,"total_b":0}
 playlist_queue=[]
 unavailable_tracks=[]
 sound=None
 playlist_last={"spotify":{},"youtube":{}}
 def build(self):
 ...
 def after_start(self,*args):
  ...
  self.submit_bug_dialog=MDDialog(title=self.loc.get("Submit bug"),text=self.loc.get("If some tracks has bad quality or even doesn't match the name you can submit it"),buttons=[MDFlatButton(text=self.loc.get("Submit bug"),on_press=lambda x:[submit_bugs(self.selected_tracks),self.submit_bug_dialog.dismiss()]),MDFlatButton(text=self.loc.get("Cancel"),on_press=lambda x:self.submit_bug_dialog.dismiss())])
  ...
  if not self.screen_manager.has_screen(screen_name):
   ...
   if "Playlist" in screen_name:
    ...
    if not hasattr(self,"localization_menu"):
     ...
     self.localization_menu_list=[{"viewclass":"OneLineListItem","height":dp(50),"text":f"{lang}","on_release":lambda x=lang:self.localization_menu_set(x)}for lang in self.loc.LANGUAGES.keys()]
    ...
    if stream:
     ...
     if track_dict_temp["state"]!=TrackStates.COMPLETED:
      if platform=="android":
       ...
       download_queue_info_temp={"position":0,"downloaded_b":0,"total_b":0}
 ...
 def mdlist_set_mode(self,instance_selection_list,mode,*args):
  if mode:
   ...
   left_action_items=[["close",lambda x:self.mdlist_set_mode(instance_selection_list,0)],["bug-outline",lambda x:self.submit_bug_dialog.open()]]
 ...
 def playlist_last_menu_show(self,youtube=False):
  ...
  self.playlist_last_menu_list=[{"viewclass":"OneLineListItem","height":dp(50),"text":f"{playlist_id}","on_release":lambda x=playlist_id:self.playlist_last_menu_set(playlist_last_dict[x],youtube)}for playlist_id in playlist_last_dict.keys()]
 ...
 def settings_save(self,notify=True):
  ...
  with open(self.settings_file_path,"w")as settings_file:
   data={"music_folder_path":self.music_folder_path,"format_mp3":self.format_mp3,"create_subfolders":self.create_subfolders,"theme":self.theme_cls.theme_style,"lang":self.loc.get_lang(),"playlist_last":self.playlist_last}
